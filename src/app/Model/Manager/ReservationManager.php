...
final class ReservationManager extends Manager
{
    ...
    /**
     * Find past reservations for admin history.
     *
     * @return array
     */
    ...
    /**
     * Find upcoming reservations in specific format for calendar render.
     *
    ...
    /**
     * Find all restricted slots in Y-m-d H:i:s format.
     *
     * @return list<string>
     */
    public function findRestrictedSlots(): array
    {
        $slots = [];
        ...
        foreach (
            ...
            $slots[] = DateTime::from($restriction->date)->format('Y-m-d H:i:s');
        }
    ...
    /**
     * Delete restricted slots in range. Removes blockers only.
     *
    ...
    {
        foreach (RestrictionSlotResolver::collectSlots($start, $end) as $slot) {
            $this->getEntityTable()
                ->where('date = ?', $slot)
                ->where('name = ?', self::RESTRICTION_NAME)
                ->delete();
    ...
    /**
     * Block slots in range. New reservations will not be allowed.
     *
     * @param DateTime $start
     * @param DateTime $end
     *
    ...
    /**
     * Cancel user reservations overlapping the restriction interval.
     *
     * @param DateTime $start
     * @param DateTime $end
     *
    ...
    {
        $this->deleteRestrictedDaysForBooking($start, $end);

        [$from, $toExclusive] = RestrictionSlotResolver::resolveUserReservationRange($start, $end);
        $result = [];

        foreach ($this->findUserReservationsInRange($from, $toExclusive)->fetchAll() as $reservation) {
     ...
     * @return Selection
     */
    private function findUserReservationsInRange(DateTime $from, DateTime $toExclusive): Selection
    {
        return $this->getEntityTable()
            ->where('date >= ?', $from)
